...
* **initial\_block\_speed**: *(float)* The player speed at the beginning of the game
* **block\_velocity\_linear**: *(float)* Degree 1 coefficient (**b**) of player speed
* **block\_velocity\_square**: *(float)* Degree 2 coefficient (**a**) of player speed

Brick speed is calculated by ax^2 + bx + c, where x is the current score of the game. It makes each round more difficult. If you want it to remain the speed every round, set **block\_velocity\_linear** and **block\_velocity\_square** to 0.
...
* **number\_of\_gaps**: *(integer)* How many gaps are in each round. It cannot be more than **brick\_count** (otherwise that'd be too easy).



###### Brick Velocity Speed Curve

(I guess this would fall under the falling bricks settings)
...
Similarly to the Player Speed Curve, the brick speed is also calculated by ax^2 + bx + c, where x is the current score of the game. It makes each round more difficult. If you want it to remain the speed every round, set **brick\_velocity\_linear** and **brick\_velocity\_square** to 0.
